...
├── model_zoo
    ├── EGNet
        ...
        │   ├──config.py                    # 解析参数配置文件
        ├── scripts
        │   ├──run_standalone_train.sh      # 启动单机训练（单卡）
        │   ├──run_distribute_train.sh      # 启动分布式训练（8卡）
        │   ├──run_eval.sh                  # 启动评估
        ...
        ├── train.py                        # 训练脚本
        ├── eval.py                         # 评估脚本
        ├── preprocess.py                   # 310评估预处理脚本
        ├── postprocess.py                  # 310评估后处理脚本
        ├── export.py                       # 模型导出脚本
...
线下运行分布式训练请参照[mindspore分布式并行训练基础样例（）](https://www.mindspore.cn/tutorials/experts/zh-CN/master/parallel/train_.html)
...
- 310处理器环境运行，需修改default_config.yaml文件：
